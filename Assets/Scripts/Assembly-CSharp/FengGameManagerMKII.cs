...
public class FengGameManagerMKII : Photon.MonoBehaviour
{
	...
	private void Update()
	{
		if (IN_GAME_MAIN_CAMERA.gametype != GAMETYPE.SINGLE && (bool)GameObject.Find("LabelNetworkStatus"))
		{
	...
	private void core()
	{
		...
		if (timeElapse > 1f)
		{
			...
			else if (IN_GAME_MAIN_CAMERA.gamemode == GAMEMODE.KILL_TITAN)
			{
				...
				text3 += "  Time : ";
				text3 = ((IN_GAME_MAIN_CAMERA.gametype != GAMETYPE.SINGLE) ? (text3 + (int)((float)time - timeTotalServer)) : (text3 + (int)timeTotalServer));
			}
			else if (IN_GAME_MAIN_CAMERA.gamemode == GAMEMODE.SURVIVE_MODE)
			{
				text3 = "Titan Left: ";
	...
	public GameObject spawnTitan(int rate, Vector3 position, Quaternion rotation, bool punk = false)
	{
		...
		GameObject gameObject2 = ((IN_GAME_MAIN_CAMERA.gametype != GAMETYPE.SINGLE) ? PhotonNetwork.Instantiate("FX/FXtitanSpawn", gameObject.transform.position, Quaternion.Euler(-90f, 0f, 0f), 0) : ((GameObject)Object.Instantiate(Resources.Load("FX/FXtitanSpawn"), gameObject.transform.position, Quaternion.Euler(-90f, 0f, 0f))));
