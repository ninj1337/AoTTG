...
public class TITAN : Photon.MonoBehaviour
{
	...
	private void findNearestFacingHero()
	{
		...
		float num2 = 0f;
		float num3 = ((abnormalType != AbnormalType.NORMAL) ? 180f : 100f);
		float num4 = 0f;
		GameObject[] array2 = array;
	...
	private string[] GetAttackStrategy()
	{
		...
		if (isAlarm || !(myHero.transform.position.y + 3f > neck.position.y + 10f * myLevel))
		{
			...
			else
			{
				...
				{
				case 2:
					array = ((!(myDistance < attackDistance * 0.25f)) ? ((!(myDistance < attackDistance * 0.5f)) ? ((abnormalType != AbnormalType.TYPE_PUNK) ? ((abnormalType != AbnormalType.NORMAL) ? new string[1] { "attack_abnormal_jump" } : ((myDifficulty <= 0) ? new string[5] { "attack_front_ground", "attack_front_ground", "attack_front_ground", "attack_front_ground", "attack_combo" } : new string[3] { "attack_front_ground", "attack_combo", "attack_combo" })) : new string[3] { "attack_combo", "attack_combo", "attack_abnormal_jump" }) : ((abnormalType != AbnormalType.TYPE_PUNK) ? ((abnormalType != AbnormalType.NORMAL) ? new string[3] { "grab_ground_front_l", "grab_ground_front_l", "attack_abnormal_jump" } : new string[3] { "grab_ground_front_l", "grab_ground_front_l", "attack_stomp" }) : new string[3] { "grab_ground_front_l", "grab_ground_front_l", "attack_abnormal_jump" })) : ((abnormalType != AbnormalType.TYPE_PUNK) ? ((abnormalType != AbnormalType.NORMAL) ? new string[1] { "attack_kick" } : new string[2] { "attack_front_ground", "attack_stomp" }) : new string[2] { "attack_kick", "attack_stomp" }));
					break;
				case 1:
					array = ((!(myDistance < attackDistance * 0.25f)) ? ((!(myDistance < attackDistance * 0.5f)) ? ((abnormalType != AbnormalType.TYPE_PUNK) ? ((abnormalType != AbnormalType.NORMAL) ? new string[1] { "attack_abnormal_jump" } : ((myDifficulty <= 0) ? new string[5] { "attack_front_ground", "attack_front_ground", "attack_front_ground", "attack_front_ground", "attack_combo" } : new string[3] { "attack_front_ground", "attack_combo", "attack_combo" })) : new string[3] { "attack_combo", "attack_combo", "attack_abnormal_jump" }) : ((abnormalType != AbnormalType.TYPE_PUNK) ? ((abnormalType != AbnormalType.NORMAL) ? new string[3] { "grab_ground_front_r", "grab_ground_front_r", "attack_abnormal_jump" } : new string[3] { "grab_ground_front_r", "grab_ground_front_r", "attack_stomp" }) : new string[3] { "grab_ground_front_r", "grab_ground_front_r", "attack_abnormal_jump" })) : ((abnormalType != AbnormalType.TYPE_PUNK) ? ((abnormalType != AbnormalType.NORMAL) ? new string[1] { "attack_kick" } : new string[2] { "attack_front_ground", "attack_stomp" }) : new string[2] { "attack_kick", "attack_stomp" }));
					break;
				case 3:
					if (myDistance < attackDistance * 0.5f)
					{
						array = ((abnormalType != AbnormalType.NORMAL) ? new string[1] { "grab_ground_back_l" } : new string[1] { "grab_ground_back_l" });
					}
					break;
				case 4:
					if (myDistance < attackDistance * 0.5f)
					{
						array = ((abnormalType != AbnormalType.NORMAL) ? new string[1] { "grab_ground_back_r" } : new string[1] { "grab_ground_back_r" });
					}
					break;
				}
			}
